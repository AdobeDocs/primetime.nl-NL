...
Zie voor oplossingen voor door de klant gemelde problemen [Opgeloste problemen](#resolved-issues). Zie voor beperkingen [bekende problemen en beperkingen](#known-issues-and-limitations).
...
De hotfix beschikte over het bijgewerkte certificaat voor individualisatietransport.
...
Hotfix voor een scenario waar het spelen tegengehouden wanneer vele verzoeken van de advertentiesolutie gelijktijdig werden gemaakt.
...
* HLS-versie van advertentie gebruikt hoger niveau dan inhoud.
...
VHL 2.0 integreren en certificeren in iOS TVSDK: de barrière in de `VideoHeartbeatsLibrary` door de complexiteit van de API&#39;s te verminderen.
...
* **Ondersteuning voor meerdere CDN&#39;s voor CRS-advertenties** TVSDK ondersteunt nu Multi-CDN voor CRS-advertenties. Door FTP-gegevens op te geven voor CRS-advertenties, kunt u andere CDN-locaties opgeven dan de standaard CDN die eigendom is van de Adobe, zoals [!DNL Akamai].

**Versie 1.4.29**

In de `PTSDKConfig` de klasse `forceHTTPS` API is toegevoegd.

De `PTSDKConfig` Deze klasse biedt methoden om SSL af te dwingen voor aanvragen die zijn ingediend bij Adobe Primetime en voor beslissings-, DRM- en Video Analytics-servers. Zie de klasse `forceHTTPS` en `isForcingHTTPS` methoden voor deze klasse. Als een manifest over HTTPS wordt geladen, behoudt TVSDK het inhoudsgebruik van HTTPS en eerbiedigt dit gebruik wanneer het laden van om het even welke relatieve URLs van dat manifest.
...
* **Video Heartbeats Library (VHL)-update naar versie 1.4.1.1**
   ...
   * Het bijhouden van advertenties is geoptimaliseerd door de `trackAdBreakStart` en `trackAdBreakComplete` methoden. Het ad-einde wordt afgeleid van het `trackAdStart` en `trackAdComplete` methode roept.
   * De `playhead` Deze eigenschap is niet meer nodig voor het bijhouden van advertenties.
   * Extra ondersteuning voor de Experience Cloud-id-service.
...
* **Ticket3531** - De status van de speler wordt niet GEPAUZEERD tijdens een onderbreking van de telefoonvraag Toegevoegde onderbreekt manager om de speler tegen te houden die onderbreekt. Na onderbreking wordt de spelerstatus gepauzeerd en wordt het afspelen hervat wanneer u op de afspeelknop klikt.
...
* **Ticket36492** - `currentTime` en `localTime` worden niet bijgewerkt wanneer het zoeken naar een nieuwe positie tijdens de huidige tijd van de gepauzeerde status Player kan nu worden ingesteld op nul voor het geval dat de speler zich in de pauzestatus bevindt. Eerder was de huidige tijd ingesteld op nul alleen in de afspeelstatus.
...
* **Ticket34683** - De afspeeltijd van de advertentie is negatief
...
* **Ticket34801** - `currentTime` en `localTime` werden niet bijgewerkt wanneer tijdens het zoeken naar een nieuwe positie tijdens de huidige tijd van de gepauzeerde status Player nu op nul kan worden ingesteld voor het geval de speler zich in de pauzestatus bevindt. Eerder was de huidige tijd ingesteld op nul alleen in de afspeelstatus.
...
* (ZD#32990) - iOS: Inhoud afspelen in plaats van op bepaalde actiepunten. `selectedMediaOptionInMediaSelectionGroup` API die deel uitmaakte van de AVPlayerItem-interface is nu onder `AVMediaSelection` in iOS 11. Het probleem is opgelost met deze nieuwe API.

* (ZD#33683) TVSDK verwijderd `==` van de reeksen metagegevens. Het probleem is opgelost in de parseringslogica.
...
* ZD (2074) - `AUDVAST` Eenmaal per minuut crashen in iOS
...
Dit probleem is opgelost door foutmelding te verbeteren als er een fatale fout is opgetreden `AVFoundation` treedt op en zorgt ervoor dat de toepassing het opnieuw opstarten kan afhandelen nadat de fout is opgetreden.
...
Dit probleem is opgelost doordat de toepassing zich kan abonneren op de `EXT-X-PROGRAM-DATE-TIME` op live streams.
...
Het neerstorten in de TVSDK waarbij een instantie van de grote parser ten onrechte werd ontkoppeld, is opgelost.
...
* (Zendesk #2231) - Fout teruggekeerd van het halen van manifest niet beschikbaar in MediaPlayerNotification

Dezelfde resolutie als Zendesk #2228

* (Zendesk #3304) - VAST 3.0 `[ERRORCODE]` macro niet gevuld

De kwestie waar de [!DNL Auditude] SDK kan niet verzenden pingelt wanneer volgende URL ruimten aan het begin heeft opgelost.
...
Deze crash gebeurt tijdens het aanroepen van `MediaCodecList` en `MediaCodecInfo` API&#39;s wanneer wordt gecontroleerd of het profiel en het niveau worden ondersteund. Adobe zoekt naar ondersteuning door Google voor meer inzicht. Dit probleem is opgelost door een tijdelijke oplossing te bieden door alle codec-informatie van tevoren te laden, zodat deze API&#39;s niet alleen hoeven te worden aangeroepen wanneer codec-informatie nodig is.
...
Opgeloste geheugen `drmFramework` met standaardweergave op iOS 8.2-apparaten
...
* tvOS
  ...
  **Opmerking**: Onthoud de volgende compilatierichtlijnen:
   ...
   * De TVSDK-tvOs-ondersteuning is beperkt tot DRM-gecodeerde streams die geen Adobe zijn. U moet de verwijzing verwijderen naar `drmNativeInterface.framework` in uw tvOS-ontwikkelinstellingen. Met AES gecodeerde stromen worden nog steeds ondersteund.
...
Oplossing: specificeer expliciet de `libstdc++.6.dylib` bibliotheek, als volgt: Ga naar **[!UICONTROL Target]** > **[!UICONTROL Build Phases]** > **[!UICONTROL Link Binary With Libraries]** en toevoegen `libstdc++.6.dylib`.
...
Opmerking: in bepaalde iOS-versies laadt het besturingssysteem de bronnen in het dialoogvenster `PSDKLibrary.framework` automatisch. Het is belangrijk dat u de `PSDKResources.bundle` naar de bundelbronnen van de toepassing: Ga naar **Fasen samenstellen** en kopieert bundelbronnen.
...
* VPAID-advertenties respecteren de `delayAdLoadingTolerance` waarde.
...
* [TVSDK iOS 3.4 API-referentie](https://help.adobe.com/en_US/primetime/api/psdk/appledoc_v34/index.html)
