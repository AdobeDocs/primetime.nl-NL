...
seo-title: Opmerkingen bij de release TVSDK 3.10 voor Android
description: Opmerkingen bij de release van TVSDK 3.10 voor Android beschrijven wat nieuw of gewijzigd is, de opgeloste en bekende problemen en de apparaatproblemen in TVSDK Android 3.10
seo-description: Opmerkingen bij de release van TVSDK 3.10 voor Android beschrijven wat nieuw of gewijzigd is, de opgeloste en bekende problemen en de apparaatproblemen in TVSDK Android 3.10
...
De huidige versie is gericht op het oplossen van de belangrijkste problemen van de klant, zoals vermeld in de [sectie Opgeloste problemen](#resolved-issues) .
...
* **Net in de tijd - Adverterend advertenties dichter aan advertentiemarkeringen**
   ...
   **Opmerking: Het luie Oplossen van Advertenties is nu veranderd om door gebrek worden uitgezet, en uitdrukkelijk moet worden toegelaten.**

   Er wordt een nieuwe API toegevoegd aan *AdvertisingMetadata::setDelayAdLoadingTolerance* om de vertraagde tolerantie bij het laden van deze advertentiemetagegevens te verkrijgen.\
   Het zoeken wordt nu meteen na de voorbereiding toegestaan, als u op ad-hocpauzes zoekt, wordt dit onmiddellijk opgelost voordat het zoeken is voltooid.\
   De signalerende wijzen SERVER_MAP en MANIFEST_CUES worden gesteund.

   Zie TVSDK 3.0 voor de Android-programmeergids voor meer informatie over API- en gebeurteniswijzigingen.

* **Bijwerken `targetSdkVersion` naar de nieuwste versie\
   **Bijwerken `targetSdkVersion` van 19 naar 27 voor een vloeiende werking.
...
In deze sectie wordt een overzicht gegeven van het probleem dat is opgelost in TVSDK 3.10 Android-release.
