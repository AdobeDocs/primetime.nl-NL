...
Er wordt een Booleaanse parameterwaarde geaccepteerd. Als de waarde ervan `True`, gebruikt de client de latentie van de audio-uitvoer in de tijdstempelberekening.
...
* Android TVSDK wordt bijgewerkt om CRS-aanvragen van de nieuwe Akamai-host in te dienen `primetime-a.akamaihd.net`.
...
* **Ad Fallback, Daisy chaining in ad selection logic (Zendesk #3103)** Voor VAST-advertenties (creatieven) waarvoor de fallback-regel is ingeschakeld, behandelt TVSDK een advertentie met een ongeldig MIME-type als een lege advertentie en probeert het gebruik van fallback-advertenties in zijn plaats. U kunt bepaalde aspecten van fallback-gedrag configureren.
...
* **Ondersteuning voor individuele support op locatie** Steun voor op-gebouw installaties van de Server van de Individualisering van de Adobe om het de individualisatieverzoek van de cliënt aan te passen om naar een verschillend eindpunt te gaan.
...
* **Integratie van Nielsen SDK** TVSDK ondersteunt nu het verzenden van informatie voor het bijhouden van gebruikers naar de Nielsen SDK zonder aangepaste integratie.
...
* Ticket #34493 - Bluetooth-audiovertraging
   ...
   * Toegevoegd `alwaysUseAudioOutputLatency` in de MediaPlayer-klasse die, wanneer deze is ingesteld, de latentie van de audio-uitvoer in de tijdstempelberekening zal gebruiken.
...
* De duur van het element Zendesk #22675 wordt niet bijgewerkt voor Live/Lineaire stromen Dit probleem is opgelost door een nieuwe API, assetDuration, op te geven in PTVideoAnalyticsTrackingMetadata die de elementduur voor Live en Lineaire stromen biedt.

* Zendesk #25853 Geheugenlek in TVSDK wanneer het schakelen van kanalen De kwestie waar een dossier-gelezen bufferlek wanneer MediaPlayer wordt teruggesteld of vrijgegeven terwijl het downloaden van een dossier is opgelost.

**Versie 1.4.29 (1653)**

* Zendesk #21200 - Player herstelt zich niet van stilstaande toestand wanneer de toepassing op de achtergrond werd uitgevoerd Wanneer de speler werd onderbroken nadat de streamschakelaar was gesignaleerd, stelt de resolutie de speler in staat de streamschakelaar uit te voeren wanneer de speler uit de status suspend wordt hersteld, in plaats van de vorige positie te herstellen.
...
* Zendesk #25078 - TVSDK DRM Geheugenlek op Android TV STB Het geheugenlek voor het DRM-adapterobject is gevonden en hersteld.

* Zendesk #25067 - Crash in VideoEngineTimeline Dit gebeurt omdat de objecten niet correct zijn schoongemaakt en gebeurtenissen werden aangeroepen nadat de objecten waren vernietigd. Het probleem is opgelost door controles toe te voegen om null-uitzonderingen te voorkomen.

* Zendesk #25352 - Vastgestelde de kopbal van douaneHTTP Dit probleem werd opgelost door een nieuwe douanekopbal aan de lijst van gewenste personen op TVSDK toe te voegen.
...
* Zendesk #23618 - Gebeurtenissen voor regeleinden branden voordat het advertentiebeleid werd geraadpleegd Dit probleem is opgelost door de gebeurtenissen AD_BREAK_START en AD_START niet te negeren wanneer de advertentie wordt overgeslagen vanwege de advertentie. De gebeurtenis AD_BREAK_SKIPPED wordt in plaats daarvan verzonden.

**Versie 1.4.27 (1631)**

* Zendesk #23174 - Prestatieprobleem bij het wijzigen van het formaat van de video Dit probleem is opgelost door een nieuwe API aan te tonen, MediaPlayerView.setSurfaceFixedSize, waarmee TVSDK toegang heeft tot SurfaceHolder.setFixedSize() vanaf MediaPlayerView.

* Zendesk #24450 - TVSDK maakt dubbele advertenties Dit probleem is opgetreden toen de verstreken tijd werd omgezet in lang en niet dubbel, en dit probleem is opgelost.
...
* Zendesk #23176 - De toepassing loopt vast wanneer het probeert VPAID-advertenties af te spelen De crash is opgetreden omdat de toepassing geen aangepaste advertentieweergave maakt om een VPAID-advertentie te renderen. Dit probleem is opgelost door de VPAID-advertenties in de reactie van de advertentieserver te negeren als er geen aangepaste advertentieweergave is.

* Zendesk #23153 - SampleAES DRM Stream - Playback stalling in de TVSDK Reference Player Dit probleem is hetzelfde als Zendesk #22900.
...
* Zendesk #18852 - Werk de creatieve selectielogica bij die op de regels van CRS wordt gebaseerd Dit probleem werd opgelost door een JSON-configuratiedossier toe te voegen om de creatieve selectieprioriteit te specificeren.

* Zendesk #20861 - Android N Deze release biedt ondersteuning voor Android N door de mogelijkheid te verwijderen om de native bibliotheken van het Android-platform rechtstreeks te gebruiken die niet meer toegankelijk zijn voor toepassingen die op Android N worden uitgevoerd.

* Zendesk #21018 - Android N loopt vast Dezelfde resolutie als ZD# 20861.

* Zendesk #21266 - VideoEngineAdapter crasht op een Invalid_Key fout Invalid_Key de fout Invalid_Key omvat geen beschrijving van AVE, zodat het ontleden van de tekst in NPE resulteerde. Het probleem is opgelost door een null-controle voor de beschrijving tijdens onError toe te voegen voordat de beschrijving werd geparseerd.

* Zendesk #22286 - De inheemse bibliotheek gaat op het toewijzen van de sleutel/het fragment van het geheugenlezing die tot neerstorting leidde Deze botsing die op Android optrad wanneer het proberen om manifest met veelvoudige sleutels gelijktijdig te laden is opgelost.

**Versie 1.4.22 (1581)**

* Zendesk #17236 - Onbetrouwbare tijd voor het afspelen van HLS-video&#39;s met DRM De tijdsprong met de LBA-streams, waarbij de begintijd van het audiosegment niet overeenkomt met de begintijd van het videosegment, is gecorrigeerd.

* Zendesk #17680 - De video playback bevriest op de doos van Selevision Andredo De videodecoder op dit apparaat keert soms een significante outputtijdsprong terug wanneer het deijden van videoframe van de outputbuffer, en dit outputtimestamp blijft hoog. Dit probleem is opgelost door een *videoprofiel wordt niet ondersteund* fout die de speler niet dwingt hetzelfde profiel opnieuw te proberen of een ander profiel te kiezen.

* Zendesk #19074 - Video bevriest tijdens FWD en REW-truc Dit probleem is opgelost door een nieuwe waarschuwing toe te voegen aan TRICKPLAY_ENDED_DUE_TO_ERROR om de toepassing te melden dat trickplay is afgesloten en dat de video is gepauzeerd vanwege een onherstelbare fout.
...
* Zendesk #19574 - De TVSDK retourneert geen M3U8-responsgegevens voor DRM- of niet-DRM-inhoud In de eerste load van het manifestbestand in PTMediaPlayerItem.prepareToPlay rapporteert de TVSDK, als het laden mislukt, de hoofdtekst van de mislukkingsreactie op de toepassing niet.
Dit probleem is opgelost door de TVSDK toe te staan de mislukkingsreactie als een fout aan de toepassing te melden.

* Zendesk #19701 - Het afspelen bevriest met SAP/discontinue De speler bevriest wanneer de audio en video niet zijn uitgelijnd bij discontinuïteit is opgelost.
...
* Zendesk #19232 - App met TVSDK 1.4.18 werkt vreemd genoeg op oudere Amazon OS versie 4. Dit probleem is opgelost door het maken van de verborgen webweergave in het initialisatieproces van de TVSDK-speler te verwijderen om conflicten te voorkomen met apparaten die Android Webview niet ondersteunen.
...
* Zendesk #19699 - TVSDK ontbreekt om tussen de ondertitelsporen van VTT te schakelen Deze kwestie werd opgelost door de spelerstortplaats te maken en manifest opnieuw te laden wanneer een spoor verandert en door het UTF8 probleem van de koordomzetting te verbeteren dat de dubbel-byte WebVTT de spoornamen van de titels beïnvloedde.
...
* Zendesk #2231 - Fout die van het halen van manifest niet beschikbaar in MediaPlayerNotification is teruggekeerd Deze kwestie werd opgelost door het antwoordlichaam van manifest te omvatten wanneer er een parseringsfout is.

* Zendesk #17703 - VideoEngineView verhindert geen schermafbeeldingen tijdens videoplayback de setSecure methode is beschikbaar sinds API 17, maar omdat API 17 4.2, 4.2.1 en 4.2.2 behandelt, is het niet bekend welke één een uitzondering zal werpen of of het apparaat specifiek is. Dit probleem is opgelost door VideoEngineView.setSecure in de catch-expressie try te plaatsen.

* Zendesk #17919 - Inhoud zoeken veroorzaakt hartslagfout Een ongeldige fout van de Positie van inputgegevens kwam voor als resultaat van de hartslagvraag die werd geproduceerd toen het zoeken werd begonnen na de pre-rol. Dit probleem is opgelost.
...
* Zendesk #17919 - Android - content seek veroorzaakt een hartslagfout Het probleem wanneer de hartslag in een foutstatus is wanneer er een zoekopdracht in een hoofdstuk is uitgevoerd, is opgelost.

* Zendesk #18053 - Adobe Primetime crasht bij Marshmallow The TVSDK crashte op Android M OS toen de TVSDK-bibliotheek neoncode gebruikte die de kleurconversie YUV -> RGB uitvoert. Het probleem is opgelost door de functies die dit probleem veroorzaken bij te werken met de niet-neonversie van de code.

* Zendesk #18072 - Android M - Toepassing loopt vast Wanneer wordt gecontroleerd of het profiel en het niveau worden ondersteund, treedt een crash op wanneer MediaCodecList en MediaCodecInfo API&#39;s worden aangeroepen. Het probleem is opgelost door tijdelijk werk te leveren door alle codec-informatie van tevoren te laden, zodat deze API&#39;s niet alleen hoeven te worden aangeroepen wanneer codec-informatie nodig is.
...
* Zendesk #17851 - Android TV - Black Frame tijdens ABR. De oplossing was om KEY_MAX_WIDTH en KEY_MAX_HEIGHT op te geven om adaptief afspelen mogelijk te maken.
...
* Zendesk #3502 - de cliënt gebaseerde steun van HLS failover tijdens een onderbreking staat failover aan belangrijkste manifest toe wanneer het bijwerken van levende profielfout tijdens de onderbrekingsperiode voorkomt.
...
* Zendesk #4450 - kwestie die de gegevens van douanemeta voor één enkele plaatsing in een inhoudsoplosser plaatst voeg een setter aan de montages van de Kans toe.
...
* Zendesk #2076 - Frequent schokkerig wanneer het spelen van video op Motorola Xoom met Android 4.0.3 Toegevoegde apparaten aan lijst van gewenste personen om hen te verhinderen om high-profile inhoud te spelen.

* Zendesk #2197 - `[Ads]` Het volgen en de fouten verzenden OperationFailedEvent met waarschuwingsbericht.
...
* Zendesk #2941 - Levende activa hebben geen &quot;0&quot;in zoekbaar waaier Voorheen was er een 3 segmentbuffer toen het zoeken aan het begin van een Levende stroom, nu is het mogelijk om tot het zeer begin van een levende stroom (d.w.z. het begin van het eerste segment) te zoeken.
...
In de Android N-release heeft Google de toegang tot deze persoonlijke bibliotheken verwijderd. Het Gingerbrood OS is momenteel wereldwijd goed voor minder dan 1% van het marktaandeel van Android OS, dus na versie 1.4.23 wordt het Gingerbrood OS niet meer ondersteund door de TVSDK.
