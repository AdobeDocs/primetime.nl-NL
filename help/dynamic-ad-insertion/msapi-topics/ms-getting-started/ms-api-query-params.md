...
# Serverqueryparameters manipuleren {#manifest-server-query-parameters}
...
## Herkende parameters {#recognized-parameters}
...
| g | Type clientapparaat | Wanneer het richten van regels afhangt van apparatentype | Zie lijst op [Clienttypen](https://adobeprimetime.zendesk.com) (vereist Zendesk-toegang) |
...
| pabimode | [Ondersteuning voor gedeeltelijke ](../../msapi-topics/ms-insert-ads/partial-ad-break-insetion.md) invoeging en onderbreking inschakelen indien waar (true) of Uitschakelen starten indien onwaar (false) | Nee (standaard is uitgeschakeld) | start, true of false |
...
| ptcdn | Lijst van één of meerdere CDNs aan gastheer transcoded activa. Zie [Ondersteuning voor meerdere CDN](../../creative-repackaging-service/multi-cdn-supportt.md). | Geen (standaard=Akamai) | Voorbeeld: Akamai, Level3, Limelight, Comcast |
| ptcueformat | De naam van de aangepaste ad-cueformaat in de M3U8. | Nee | DPISimple, DPIScte35, Element,NBC, NFL of Turner |
| ptfailover | Signaleert de manifestserver om primaire en failover stromen te identificeren die in master playlist worden gespecificeerd, en hen te behandelen als gescheiden reeksen. Dit vergemakkelijkt failover en voorkomt timingsfouten. (Alleen voor Apple HLS-apparaten.) Zie [HLS-speler-switching vergemakkelijken](../../msapi-topics/ms-insert-ads/hls-switching-to-failover.md). | Nee | true |
| ptmulticall | Indien ingesteld op true, worden meerdere controles ad-call voor FER uitgevoerd; één voor elke advertentie-onderbreking.  Indien afwezig of geplaatst aan vals, wordt één ad-call gemaakt om voor FER te controleren. | Nee | Boolean   Opmerking:  De volgende eisen: <ul><li>ptcueformat-parameter moet op nbc worden ingesteld</li><li>parameter pttimeline wordt genegeerd.</li></ul> |
...
| pttimeline | Een tekenreeks met de gewenste tijdlijn voor plaatsing en inhoud van de advertentie, die de inhoudsstroom overschrijft en afbreekt. | Nee | VOD-tijdlijn (zie [Tijdlijnindeling VOD](../../msapi-topics/ms-changes-vod-timeline/ms-api-timeline-format.md)) |
