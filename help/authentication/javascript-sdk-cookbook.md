...
- Adobe Primetime-verificatiebibliotheek (AccessEnabler), werkt u samen met uw Adobe Primetime-verificatieaccountmanager om dit te regelen.
...
- `setAuthenticationStatus(status, errorcode)`
   ...
   **Trigger:**
   ...
   De geretourneerde status is geslaagd of mislukt. In de foutcode wordt het type fout beschreven.
...
- `sendTrackingData(event, data)`
   ...
   **Triggers:** `checkAuthentication(), getAuthentication(),checkAuthorization(), getAuthorization(), setSelectedProvider()`.  De `event` parameter geeft aan welke gebeurtenis machtiging heeft plaatsgevonden; de `data` parameter is een lijst van waarden met betrekking tot de gebeurtenis. 
- `setToken(token, resource)`

   **Trigger:** `checkAuthorization()`en `getAuthorization()` na een geslaagde machtiging om een bron te bekijken.   De `token` parameter het kortstondige media-token is; de `resource` parameter is de inhoud die de gebruiker mag bekijken.

- `tokenRequestFailed(resource, code, description)`

   **Trigger:**`checkAuthorization()` en`getAuthorization()`  na een geslaagde toelating.\
...
- `setMetadataStatus(metadata, key, arguments)`
   ...
   **Trigger:** `getMetadata().`\
   De `metadata` parameter de specifieke gegevens verstrekt u vroeg; de belangrijkste parameter is de sleutel die in `getMetadata()`verzoek; en de `arguments` parameter is hetzelfde woordenboek als dat waaraan is doorgegeven `getMetadata()`.


## 2. Stroom opstarten

**I. Laad JavaScript van AccessEnabler:**
...
 
**II.** Bellen `setRequestor()`de identiteit van de programmeur vast te stellen; slagen in de `requestorID` en (optioneel) een array met Adobe Primetime-verificatieeindpunten.
...
- Als de vraag overgaat: De gebruiker heeft een geldig token AuthN en de gebruiker is geautoriseerd om de gevraagde media te bekijken.
...
- Als de validatie slaagt: De gewenste media voor de gebruiker weergeven/afspelen.
- Indien mislukt: De AuthZ-token was ongeldig, de mediaquery moest worden geweigerd en er moet een foutbericht worden weergegeven aan de gebruiker.

## 5. Media Flow weergeven (#logout)
